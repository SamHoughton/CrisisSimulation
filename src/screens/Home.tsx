...
export function Home() {
  ...
  return (
    <div className="min-h-full">
      ...
      {/* ── Hero ──────────────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden border-b border-crux-border">
        ...
        {/* Top accent line */}
        <div className="h-[2px] bg-gradient-to-r from-crux-green via-crux-green/60 to-transparent" />
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] text-crux-dim tracking-[0.35em] uppercase font-medium">Crisis Simulation Platform</span>
              </div>
              <h1 className="text-3xl font-bold text-crux-text mb-2">
                Prepare your team for the
                <span className="text-crux-red ml-2">worst day</span>
              </h1>
              <p className="text-sm text-crux-muted max-w-lg leading-relaxed">
              ...
              <div className="flex items-center gap-3 mt-5">
                <button
                  ...
                  className="flex items-center gap-2 bg-crux-green text-white px-4 py-2.5 rounded-lg text-xs font-semibold hover:brightness-110 transition"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-crux-elevated text-crux-muted px-4 py-2.5 rounded-lg text-xs font-medium border border-crux-border hover:text-crux-text hover:border-crux-border-light transition"
                >
                ...
                <a
                  ...
                  className="flex items-center gap-1.5 text-xs text-crux-dim hover:text-crux-muted transition"
                >
      ...
      <div className="px-8 max-w-5xl mx-auto py-8">
        ...
        {session && session.status !== "ended" && (
          <div className="mb-8 fade-in-up">
            <button
              ...
              className="w-full flex items-center gap-4 bg-crux-red/8 border border-crux-red/30 rounded-xl p-4 hover:bg-crux-red/12 transition-colors text-left group"
            >
              <div className="relative shrink-0">
                <AlertTriangle className="w-5 h-5 text-crux-red" />
                <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-crux-red animate-ping" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold text-crux-text">{session.scenario.title}</p>
                <p className="text-xs text-crux-muted mt-0.5">
                  Session {session.status} · {session.liveInjects.length}/{session.scenario.injects.length} injects released
                </p>
              </div>
              <span className="text-xs font-semibold text-crux-red bg-crux-red/15 px-2 py-0.5 rounded-full font-mono pulse-dot">
                {session.status === "active" ? "LIVE" : session.status.toUpperCase()}
              </span>
              <ChevronRight className="w-4 h-4 text-crux-dim group-hover:translate-x-0.5 transition-transform" />
            </button>
          </div>
        )}
        ...
        <section className="mb-10">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-semibold text-crux-dim uppercase tracking-wider">
              Scenario Templates
            </h2>
            <button onClick={() => setView("library")} className="text-xs text-crux-green hover:underline flex items-center gap-1">
        ...
        {/* ── How It Works ────────────────────────────────────────────────── */}
        <section className="mb-10">
          <h2 className="text-xs font-semibold text-crux-dim uppercase tracking-wider mb-4">
            How It Works
          </h2>
        ...
        {allScenarios.filter((s) => s.isTemplate).length === 0 && (
          <div className="text-center py-12 border border-dashed border-crux-border rounded-xl mb-8 fade-in-up">
            <Zap className="w-8 h-8 text-crux-dim mx-auto mb-3" />
            <p className="text-sm font-medium text-crux-text mb-1">No scenarios yet</p>
            <p className="text-xs text-crux-muted mb-4">Create your first crisis scenario to get started</p>
            <button
              onClick={() => { useStore.getState().setEditingScenario(null); setView("builder"); }}
              className="inline-flex items-center gap-2 bg-crux-green text-white px-4 py-2 rounded text-xs font-medium hover:brightness-110 transition"
            >
        ...
        {pastSessions.length === 0 && (
          <section className="mb-8 fade-in-up">
            <p className="text-xs text-crux-dim text-center mb-2">Returning from a previous session?</p>
            <label className="flex items-center justify-center gap-1.5 text-xs text-crux-muted hover:text-crux-text border border-dashed border-crux-border px-3 py-2.5 rounded-lg transition-colors cursor-pointer mx-auto w-fit">
        ...
        {recent.length > 0 && (
          <section className="fade-in-up">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs text-crux-muted hover:text-crux-text border border-crux-border px-2.5 py-1.5 rounded transition-colors"
                  title="Export all sessions as JSON"
                >
                  <Download className="w-3 h-3" />Export
                </button>
                <label className="flex items-center gap-1.5 text-xs text-crux-muted hover:text-crux-text border border-crux-border px-2.5 py-1.5 rounded transition-colors cursor-pointer"
            ...
            <div className="border border-crux-border rounded-xl divide-y divide-crux-border overflow-hidden bg-crux-panel">
              {recent.map((s) => (
                <button
                  key={s.id}
                  onClick={() => {
                  ...
                  className="w-full flex items-center gap-4 px-5 py-4 hover:bg-crux-elevated transition-colors text-left group"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-crux-text truncate">{s.scenario.title}</p>
                    <p className="text-xs text-crux-muted mt-0.5">
                  ...
                  {s.report ? (
                    ...
                    <span className="text-xs text-crux-dim bg-crux-elevated px-2 py-0.5 rounded-full">
                      No report
                    </span>
                  )}
                  <ChevronRight className="w-4 h-4 text-crux-dim group-hover:translate-x-0.5 transition-transform" />
        ...
        {/* ── Open source footer ──────────────────────────────────────────── */}
        <div className="mt-12 pt-6 border-t border-crux-border flex items-center justify-between">
          <p className="text-[11px] text-crux-dim leading-relaxed max-w-md">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 text-xs text-crux-dim hover:text-crux-muted border border-crux-border hover:border-crux-border-light px-3 py-2 rounded-lg transition-colors shrink-0 ml-8"
          >
            <Github className="w-3.5 h-3.5" />
            View on GitHub
          </a>
...
function StatCard({
  ...
  return (
    <div className="bg-crux-panel/60 border border-crux-border rounded-xl px-5 py-4 min-w-[100px] text-center backdrop-blur-sm">
      <div className="flex justify-center mb-2 text-crux-dim">{icon}</div>
      <p className="text-xl font-bold text-crux-text font-mono">{displayed}{suffix}</p>
      <p className="text-[10px] text-crux-dim uppercase tracking-wider mt-0.5">{label}</p>
    </div>
  );
}
...
function StepCard({
  ...
  return (
    <div className="bg-crux-panel border border-crux-border rounded-xl p-5 group hover:border-crux-border-light transition-colors">
      <div className="flex items-center gap-3 mb-3">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center text-crux-green"
          style={{ background: "rgba(29,184,106,0.08)", border: "1px solid rgba(29,184,106,0.15)" }}>
          {icon}
        </div>
        <span className="text-[10px] font-bold text-crux-dim tracking-wider uppercase">Step {step}</span>
      </div>
      <p className="text-sm font-semibold text-crux-text mb-1.5">{title}</p>
      <p className="text-xs text-crux-muted leading-relaxed">{description}</p>
    </div>
  );
}
...
function ScorePill({ score }: { score: number }) {
  const colour =
    score >= 75 ? "text-crux-green bg-crux-green/10 border border-crux-green/20" :
