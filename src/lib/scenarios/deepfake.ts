...
export const DEEPFAKE_SCENARIO: Scenario =
  {
    ...
    realOutcome: "Deepfake CEO fraud has affected multiple FTSE-listed organisations since 2023. In the most-cited UK incident, a finance team at a multinational was socially engineered into transferring approximately £20 million following a deepfake video call in which every participant except the victim was AI-generated.\n\nFor listed companies facing a viral deepfake, the core regulatory tension is between the FCA's DTR 2.2 (obligation to announce price-sensitive information without delay) and the practical need to authenticate before issuing any statement. Issuing a denial of a video you have not yet technically verified exposes the company to a different set of regulatory consequences if the verification later fails.\n\nOrganisations that handled deepfake crises most effectively shared three characteristics: a pre-agreed out-of-band authentication protocol (a personal code phrase known only to the real executive); a designated first-responder team with pre-approved draft denial language; and a direct line to their FCA supervisor and financial PR advisor established before any public statement. Those that improvised typically issued multiple contradictory statements within 90 minutes, compounding the market impact.",
